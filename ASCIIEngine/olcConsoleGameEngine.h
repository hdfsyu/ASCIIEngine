...
keycode you want to know about. pressed is set for the frame the key is pressed down
...
Modified by @hdfsyu on github for easier variable names
*/
...
public:
	...
	int ConstructConsole(int width, int height, int fontw, int fonth)
	{
		...
		COORD coord = { (short)m_nsw, (short)m_nsh };
	...
	/*	DWORD version = GetVersion();
		...
		if (m_nsh > csbi.dwMaximumWindowSize.Y)
			return Error(L"Screen Height / Font Height Too Big");
		if (m_nsw > csbi.dwMaximumWindowSize.X)
			return Error(L"Screen Width / Font Width Too Big");

		// Set Physical Console Window Size
		m_rectWindow = { 0, 0, (short)m_nsw - 1, (short)m_nsh - 1 };
		...
		m_bufScreen = new CHAR_INFO[m_nsw*m_nsh];
		memset(m_bufScreen, 0, sizeof(CHAR_INFO) * m_nsw * m_nsh);
	...
	{
		if (x >= 0 && x < m_nsw && y >= 0 && y < m_nsh)
		{
			m_bufScreen[y * m_nsw + x].Char.UnicodeChar = c;
			m_bufScreen[y * m_nsw + x].Attributes = col;
		}
	}
	...
	void DrawString(int x, int y, std::wstring c, short col = 0x000F)
	{
		...
		{
			m_bufScreen[y * m_nsw + x + i].Char.UnicodeChar = c[i];
			m_bufScreen[y * m_nsw + x + i].Attributes = col;
		}
	}
			...
			while (m_bAtomActive)
			{
				...
				for (int i = 0; i < 256; i++)
				{
					...
					if (m_keyNewState[i] != m_keyOldState[i])
					{
						...
						{
							m_keys[i].pressed = !m_keys[i].bHeld;
							m_keys[i].bHeld = true;
						}
						else
						{
				...
				wchar_t s[256];
				swprintf_s(s, 256, L"hdfsyu's console game engine - %s - FPS: %3.2f", m_sAppName.c_str(), 1.0f / fElapsedTime);
				SetConsoleTitle(s);
				WriteConsoleOutput(m_hConsole, m_bufScreen, { (short)m_nsw, (short)m_nsh }, { 0,0 }, &m_rectWindow);
			}
...
public:
	...
	virtual bool ouc()							= 0; //on user create
	virtual bool oup(float fElapsedTime)		= 0; //on user update

	// Optional for clean up
	virtual bool oud()						{ return true; } //on user destroy
